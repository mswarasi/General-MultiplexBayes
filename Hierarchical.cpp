...
// [[Rcpp::export]]
double EFF(vec p, int S, mat SE, mat SP, vec ns){ 
...
// [[Rcpp::export]]
List ACCU(vec p, int S, mat SE, mat SP, vec ns){ 
...
List OPT(vec p, mat SE, mat SP,int MAMPS,string obj,string algorithm){
  mat efficiency(5,10,fill::zeros); mat ps(5,6,fill::zeros);
  for (int S=2; S<=6; S++){
    mat pools = poolsize(MAMPS,S,algorithm); 
    int num_pools = pools.n_rows; 
    mat temp(num_pools,2,fill::zeros);
     for (int i=0; i<num_pools; i++){
       temp(i,0)=EFF(p,S,SE(span((S+1)*(S-2)/2,(S+1)*(S-2)/2+S-1),span::all),SP(span((S+1)*(S-2)/2,(S+1)*(S-2)/2+S-1),span::all),trans(pools(i,span::all)));
       temp(i,1)=ACCU(p,S,SE(span((S+1)*(S-2)/2,(S+1)*(S-2)/2+S-1),span::all),SP(span((S+1)*(S-2)/2,(S+1)*(S-2)/2+S-1),span::all),trans(pools(i,span::all)))["ACCU"];
     }
     ...
     if (obj=="minimize"){
       ...
       List mid1=ACCU(p,S,SE(span((S+1)*(S-2)/2,(S+1)*(S-2)/2+S-1),span::all),SP(span((S+1)*(S-2)/2,(S+1)*(S-2)/2+S-1),span::all),trans(pools(q1d,span::all)));
    ...
    for (int j=i;j<size;j++)
      ...
      if (min(p)>=0 && max(p)<=1 )
      {
		   	...
		   	for (int k=0;k<num_config2;k++)
		    	{
			      two(k)=EFF(p,2,SE(span(0,1),span::all),SP(span(0,1),span::all),trans(pool2(k,span::all)));
            two1(k)=ACCU(p,2,SE(span(0,1),span::all),SP(span(0,1),span::all),trans(pool2(k,span::all)))["ACCU"];
			     }
         ...
         for (int k=0;k<num_config3;k++)
		    	{
            three(k)=EFF(p,3,SE(span(2,4),span::all),SP(span(2,4),span::all),trans(pool3(k,span::all)));
            three1(k)=ACCU(p,3,SE(span(2,4),span::all),SP(span(2,4),span::all),trans(pool3(k,span::all)))["ACCU"];
			     }
         ...
         min_stage(1)=min(three);
	       ...
	       for (int k=0;k<num_config4;k++)
  	    	{
            four(k)=EFF(p,4,SE(span(5,8),span::all),SP(span(5,8),span::all),trans(pool4(k,span::all)));
            four1(k)=ACCU(p,4,SE(span(5,8),span::all),SP(span(5,8),span::all),trans(pool4(k,span::all)))["ACCU"];
  		     }
        ...
        for (int k=0;k<num_config5;k++)
  	    	{
            five(k)=EFF(p,5,SE(span(9,13),span::all),SP(span(9,13),span::all),trans(pool5(k,span::all)));
            five1(k)=ACCU(p,5,SE(span(9,13),span::all),SP(span(9,13),span::all),trans(pool5(k,span::all)))["ACCU"];
    			}
        ...
        for (int k=0;k<num_config6;k++)
  	    	{
            six(k)=EFF(p,6,SE(span(14,19),span::all),SP(span(14,19),span::all),trans(pool6(k,span::all)));
            six1(k)=ACCU(p,6,SE(span(14,19),span::all),SP(span(14,19),span::all),trans(pool6(k,span::all)))["ACCU"];
      		}
